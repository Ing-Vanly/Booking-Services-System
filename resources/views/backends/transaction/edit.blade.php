{{-- filepath: /c:/laragon/www/booking-service-system/resources/views/backends/transaction/edit.blade.php --}}
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="col-md-8">
                    <form id="editForm" method="POST" action="{{ route('admin.transaction.update', $transaction->id) }}"
                                    ...
                                    <div class="product-item row mb-3">
                                        ...
                                        <div class="col-md-2">
                                            <input type="text" name="discount[]" class="form-control"
                                                placeholder="{{__('Discount')}}" oninput="calculateTotals()" value="0" required>
                                        </div>
                                        <div class="col-md-1">
                                        ...
                                        <div class="product-item row mb-3">
                                            ...
                                            <div class="col-md-2">
                                                <input type="number" name="qty[]" class="form-control"
                                                    placeholder="{{__('Quantity')}}" value="{{ $sellLine->qty }}"
                                                    oninput="calculateTotals()" required>
                                            </div>
                                            <div class="col-md-2">
                                                <select name="discount_type[]" class="form-control"
                                            ...
                                            <div class="col-md-2">
                                                <input type="text" name="discount[]" class="form-control"
                                                    placeholder="{{__('Discount')}}" value="{{ $sellLine->discount_amount }}"
                                                    oninput="calculateTotals()" required>
                                            </div>
                                            <div class="col-md-1">
                                                <button type="button" class="btn btn-danger btn-sm remove-product">
                            ...
                            <div class="card-body bg-light">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="date" name="start_date" id="start_date"
                                                class="form-control" value="{{ $transaction_selllines->start_date ?? '' }}"
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="date" name="end_date" id="end_date"
                                                class="form-control" value="{{ $transaction_selllines->end_date ?? '' }}"
                                                oninput="updateReport()">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                ...
                                <div class="text-right mt-3">
                                    <button type="submit" class="btn btn-primary">
                                        <i class="fas fa-save mr-1"></i> {{__('Update Transaction')}}
                                    </button>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="info-box bg-light">
                                <div class="info-box-content">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <h5 id="report-customer" class="mb-0">{{ $transaction->customer->first_name }}
                                            {{ $transaction->customer->last_name }}</h5>
                                    </div>
                                    <div class="mb-3">
                                        <span class="text-muted">{{__('Start Date')}}</span>
                                        <h5 id="report-start-date" class="mb-0">{{ $transaction_selllines->start_date ?? '' }}</h5>
                                    </div>
                                    <div class="mb-3">
                                        <span class="text-muted">{{__('End Date')}}</span>
                                        <h5 id="report-end-date" class="mb-0">{{ $transaction_selllines->start_date ?? '' }}</h5>
                                    </div>
                                    <div class="mb-3">
...
@push('js')
    <script>
        ...
        // Update the transaction report with current form values
        function updateReport() {
            ...
            // Update the product list in the report table
        ...
        // Calculate the total amounts for the transaction
        ...
        // Validate product rows and submit the form
        document.getElementById('editForm').addEventListener('submit', function (e) {
            // Validate that for every product row with a selected product the quantity is filled and greater than 0.
            ...
            productRows.forEach(row => {
                ...
                if (productId && (qty.trim() === '' || parseFloat(qty) <= 0)) {
                    ...
                    row.style.border = ''; // Reset the border if valid.
                }
            });
            if (missingQty) {
                e.preventDefault();
                Swal.fire({
                    ...
                    text: 'Please fill in the quantity for all selected products or remove the product row.'
                });
                return;
            }

            // If validation passes, build the sell_lines array.
            e.preventDefault();
            ...
            document.querySelectorAll('.product-item').forEach(product => {
                ...
                if (productId) {
                    ...
                    const discountType = product.querySelector('[name="discount_type[]"]').value;
                    const discount = parseFloat(product.querySelector('[name="discount[]"]').value) || 0;
                    const basePrice = productSelect.options[productSelect.selectedIndex].dataset.price ? parseFloat(productSelect.options[productSelect.selectedIndex].dataset.price) : 0;
                    const lineProductTotal = qty * basePrice;
                    let discountAmount = 0;
                    if (discountType === 'percent') {
                        discountAmount = (lineProductTotal * discount) / 100;
                    } else if (discountType === 'fix') {
                        discountAmount = discount * qty;
                    }
                    const finalLineTotal = Math.max(lineProductTotal - discountAmount, 0);
                    sellLines.push({
                        product_id: parseInt(productId),
                        qty: qty,
                        discount_type: discountType,
                        discount_amount: discount,
                        sub_total: lineProductTotal.toFixed(2),
                        final_total: finalLineTotal.toFixed(2)
                    });
                }
            });
            ...
            .then(responseData => {
                ...
                if (responseData.status === 1) {
                    window.location.href = "{{ route('admin.transaction.index') }}" + "?msg=" + encodeURIComponent(responseData.msg);
                } else {
