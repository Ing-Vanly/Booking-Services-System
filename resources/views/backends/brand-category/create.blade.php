<div class="modal-dialog modal-md modal-dialog-centered">
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title">{{ __('Create Model') }}</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">×</span>
            </button>
        </div>
        <form method="POST" action="{{ route('admin.brand-category.store') }}" class="submit-form" enctype="multipart/form-data">
                                ...
                                @if ($lang['status'] == 1)
                                    <div class="tab-pane fade {{ $lang['code'] == $default_lang ? 'show active' : '' }} mt-3" id="lang_{{ $lang['code'] }}" role="tabpanel" aria-labelledby="lang_{{ $lang['code'] }}-tab">
                                        <div class="form-group">
                                            ...
                                            <input type="name" id="name_{{ $lang['code'] }}" class="form-control"
                                                name="name[]" placeholder="{{__('Enter Name')}}" value="{{ old('name.' . $lang['code']) }}" {{ $lang['code'] == 'en' ? 'required' : '' }}>
                                        </div>
                                    </div>
                                @endif
                            @endforeach
